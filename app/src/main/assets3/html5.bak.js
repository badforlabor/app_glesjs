...
document.root = HTMLParser.htmlToDomTree(_utils.loadStringAsset("index.html"),
	document._elementsById);

document.head = _findNodeName(document.root,"HEAD");
document.body = _findNodeName(document.root,"BODY");
...
function _mouseMoveCallback(x,y) {
	...
	if (_canvas._mouseMoveCallback) {
		_canvas._mouseMoveCallback(
			new _MouseEvent(_canvas._virtmousex,_canvas._virtmousey) );
	}
}
...
function _documentLoaded(width,height) {
	...
	_execOnLoad(document.root);
	if (_canvas._onloadhandler) _canvas._onloadhandler();
	console.log("Document loaded: "+width+"x"+height);
}
